...
class PaymentSessionModel {
  ...
  factory PaymentSessionModel.fromJson(
    ...
    final gatewayOrderId =
        data['gateway_order_id']?.toString() ??
        data['cashfree_order_id']?.toString() ??
        data['cf_order_id']?.toString() ??
        data['order_token']?.toString() ??
        data['razorpay_order_id']?.toString();
    final paymentSessionId =
        data['payment_session_id']?.toString() ??
        data['cashfree_payment_session_id']?.toString() ??
        data['session_id']?.toString();
    final gateway =
        data['gateway']?.toString() ??
        data['payment_gateway']?.toString() ??
        (paymentSessionId != null && paymentSessionId.isNotEmpty
            ? 'cashfree'
            : 'razorpay');

    return PaymentSessionModel(
      orderId:
          int.tryParse((data['order_id'] ?? orderId).toString()) ?? orderId,
      gateway: gateway,
      ...
      cashfreeEnv:
          data['cashfree_env']?.toString() ??
          data['environment']?.toString() ??
          'production',
      cashfreeAppId:
          data['cashfree_app_id']?.toString() ?? data['app_id']?.toString(),
      razorpayOrderId:
          data['razorpay_order_id']?.toString() ??
          data['gateway_order_id']?.toString(),
      razorpayKeyId:
          data['razorpay_key_id']?.toString() ??
          data['key_id']?.toString() ??
          data['key']?.toString(),
      amountPaise: int.tryParse(data['amount_paise']?.toString() ?? ''),
      amount: amount > 0
          ? amount
          : ((data['amount_paise'] != null)
                ? (double.tryParse(data['amount_paise'].toString()) ?? 0) / 100
                : 0.0),
      currency: data['currency']?.toString() ?? 'INR',
      checkoutUrl:
          data['checkout_url']?.toString() ?? data['payment_url']?.toString(),
      keyId:
          data['key_id']?.toString() ??
          data['razorpay_key_id']?.toString() ??
          data['key']?.toString(),
      ...
      customerPhone:
          customer?['contact']?.toString() ?? customer?['phone']?.toString(),
      rawResponse: json,
    );
  }
}
...
class PaymentVerificationResult {
  ...
  factory PaymentVerificationResult.fromJson(Map<String, dynamic> json) {
    ...
    final data =
        _firstMap(json, const ['data', 'payment', 'verification', 'order']) ??
        json;
    ...
    return PaymentVerificationResult(
      ...
      enrollmentConfirmed:
          data['enrollment_confirmed'] == true ||
          data['enrolled'] == true ||
  ...
  factory PaymentGatewayInfo.fromJson(Map<String, dynamic> json) {
    ...
    return PaymentGatewayInfo(
      ...
      name:
          json['label']?.toString() ??
          json['name']?.toString() ??
          (json['id']?.toString() == 'cashfree' ? 'Cashfree' : 'Razorpay'),
...
class MyOrderItem {
  ...
  factory MyOrderItem.fromJson(Map<String, dynamic> json) {
    return MyOrderItem(
      orderId:
          int.tryParse(
            json['id']?.toString() ?? json['order_id']?.toString() ?? '0',
          ) ??
          0,
      title:
      ...
      amount:
          double.tryParse(
            (json['total_amount'] ?? json['amount'] ?? json['price'] ?? 0)
                .toString(),
          ) ??
          0.0,
      ...
      date: DateTime.tryParse(
        json['created_at']?.toString() ?? json['order_date']?.toString() ?? '',
      ),
      productType: json['product_type']?.toString() ?? 'course',
      invoiceNumber: json['invoice_number']?.toString(),
      invoiceUrl:
          json['invoice_url']?.toString() ?? json['invoice_pdf']?.toString(),
    );
  }
}
